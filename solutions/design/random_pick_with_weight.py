...
if __name__ == "__main__":
    # pickIndex is randomised, so there is no fixed expected output.
    # Sanity check: with w = [1, 3], index 1 should come up about 75% of the time.
    from collections import Counter

    try:
        picks = Counter(Solution([1, 3]).pickIndex() for _ in range(4000))
    except NotImplementedError:
        raise SystemExit("not implemented yet - solve it first, then paste your code in")
    share = picks[1] / 4000
    print(f"  index 1 chosen {share:.1%} of the time (want ~75%)")
    print(f"\n{'pass' if 0.70 < share < 0.80 else 'FAIL'}")
